...
func NewHandler(usersService UsersService, auditService audit.AuditService, permissionsService PermissionsService, utilsInstance *utils.Utils) *Handler {
	return &Handler{
		usersService:       usersService,
...
func (h *Handler) UpdateUserPhoneNumbers(w http.ResponseWriter, r *http.Request) {
	...
	if len(req.Phones) > 0 {
		...
		for i := range req.Phones {
			...
			formattedPhone, err := utils.ValidatePhoneForCountry(req.Phones[i].RawInput, countryIso2)
			if err != nil {
